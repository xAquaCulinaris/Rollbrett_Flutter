    ...
    return Column(
      children: [
        Row(
          children: [
            IconButton(
                icon: Icon(!SkateDiceModelController.of(context).skateDiceMap[widget.index]['isExpanded'] ? Icons.arrow_right : Icons.arrow_drop_down),
                onPressed: () {
                  setState(() {
        ...
        SkateDiceModelController.of(context).skateDiceMap[widget.index]['isExpanded']
            ? ListView.builder(
                ...
                itemBuilder: (BuildContext context, int index) => SkateDiceCheckBox(SkateDiceModelController.of(context).skateDiceMap, widget.index, index))
            : Text(""),
      ],
    );
  }
